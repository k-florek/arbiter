//functions for scanning directories for new runs
const fs = require('fs');
const path = require('path');
const config = require('../config.json');

module.exports.scanForRuns = scanForRuns

//constants used for all functions
const run_directory = config.run_dir;

function scanForRuns (page,res) {

  //handle errors
  function errors(err){
    if (err) {
      return console.error(err.message);
    }
  }

  console.log('Scanning for new runs.');

  //scan run directory for new runs
  fs.readdir(run_directory,iterateFiles);

  //iterate over the files and find new runs
  function iterateFiles (err,files){
    let rows = [];
    errors(err);
    //loop through each file
    for (let i = 0; i<files.length;i++){
      //initalize vars
      let p = path.join(run_directory,files[i]);
      let stat = fs.statSync(p);
      //check if dir
      if (stat && stat.isDirectory()) {
          if (files[i].includes('WI-')) {
            //console.log(file);
            let machine = files[i].split('-')[1];
            let date = files[i].split('-')[2];
            rows.push([machine,date,p]);
          }
      }
    }
    insertDB(rows.shift());

    //insert into database
    function insertDB (row){
      //insert row into database
      let sql = `INSERT or IGNORE INTO seq_runs (MACHINE,DATE,PATH) VALUES (?,?,?)`;
      if (row){
        db.run(sql,row,(err)=>{
          errors(err);
          //remove first item and call again
          return insertDB(rows.shift());
        });
      }else{
        let sql = `SELECT MACHINE,DATE,PATH FROM seq_runs ORDER BY DATE DESC`;
        db.all(sql,renderPage);
      }
    }
  }
